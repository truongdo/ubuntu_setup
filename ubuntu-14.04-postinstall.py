...
def showexec(description, command, exitonerror = 0, presskey = 0, waitmessage = ""):
    ...
    if "setright" in description:
        command = command.replace("LOCAL_USER", user_name)
    ...
    # Run the command
    returncode = os.system ("set -x; /bin/sh -c \"%s\" >> /dev/null 2>&1" % command)
...
def main(argv):
    ...
    if (config.has_section("dotfiles")):
        ...
        showexec (_("Install the bash configuration file"), "chown -R %s $HOME/.bashrc*"%user_name)
        # Vim
        if (config.has_option("dotfiles", "vimrc")):
            ...
            showexec (_("Install the Vim configuration file"), "chown -R %s $HOME/.vimrc"%user_name)
